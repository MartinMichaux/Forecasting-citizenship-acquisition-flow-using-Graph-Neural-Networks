...
class get_data():
    ...
    def merge_data(self):
        if self.based_on!=None:
            ...
            self.feature_names = self.df_merged.drop(columns=["Country_x","Country_y","COU", "CO2"]).columns.tolist()
        
        
    def create_X_y(self,q=5,mode="class"):
        if (self.based_on==None) and ("Country_x" in self.df_merged.columns):
